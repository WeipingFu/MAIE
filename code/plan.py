...
class PlanAgent(Agent):
    ...
    def get_userprompt(self, task:str, model_outputs:list) -> None:
        ...
        # generate prompt
        self.user_prompt = self.prompt_template.replace('#task_description', task).replace('#evaluation_mode', eval_mode).replace('#model_output', model_output).replace('#criteria', self.user_criteria).replace('#examples', example_str)

    def apply_one(self, task:str, model_outputs:list, pre_messages:list = None) -> str:
        self.get_userprompt(task, model_outputs)
...
if __name__ == "__main__":
    ...
    planer.apply_one(task, model_outputs, pre_messages=None)